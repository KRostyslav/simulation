 ...
 */
export function builtInVoltage({ na, nd, temperature, material = SI }) {
 ...
 */
export function depletion({ na, nd, temperature, bias = 0, material = SI }) {
  const vbi = builtInVoltage({ na, nd, temperature, material });
  const vt = thermalVoltage(temperature);
  const raw = vbi - bias;
  ...
  const width = Math.sqrt(((2 * material.eps * veff) / Q) * (1 / na + 1 / nd));
  ...
  return {
    ...
    capacitancePerArea: material.eps / width, // Ф/см² — варікап
  };
}

/** Місткість переходу [Ф] для площі A. Основа варікапа: C ~ 1/√(V_bi − U). */
export function junctionCapacitance({ na, nd, temperature, bias, area, material = SI }) {
  return depletion({ na, nd, temperature, bias, material }).capacitancePerArea * area;
}
