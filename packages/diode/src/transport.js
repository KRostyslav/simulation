/**
 * Перенесення заряду: рухливість, провідність, дифузія.
 *
 ...
 * розсіює носії. У власному кристалі перший ефект на порядки сильніший —
...
import { Q, SI, T0 } from "./constants.js";
 ...
 */
export function electronMobility(totalDoping, temperature, material = SI) {
  const { muNmin, muNmax, nRefN, alphaN, tExpN } = material;
...
export function holeMobility(totalDoping, temperature, material = SI) {
  const { muPmin, muPmax, nRefP, alphaP, tExpP } = material;
 ...
 */
export function driftVelocity(mobility, field, material = SI) {
  const low = mobility * field;
  return low / (1 + Math.abs(low) / material.vSat);
}
